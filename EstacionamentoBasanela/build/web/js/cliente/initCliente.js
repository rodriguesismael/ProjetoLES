...
function ativarCliente(cpf) {
    alertify.confirm("Deseja realmente ativar este cliente?", function(r) {
        if (r) {
            $.ajax({
                url: "Controller?name=AtivarCliente",
                type: "POST",
                contentType: "application/x-www-form-urlencoded; charset=UTF-8",
                data: {
                    cpf: cpf
                ...
                success: function(html) {
                    $("#pagina").html("");
                    $("#pagina").html(html);
                    alertify.log("Operação concluída com sucesso!", "success", 5000);
                }
            });
        }
    });
...
function inativarCliente(cpf) {
    alertify.confirm("Deseja realmente inativar este cliente?", function(r) {
        if (r) {
            $.ajax({
                url: "Controller?name=InativarCliente",
                type: "POST",
                contentType: "application/x-www-form-urlencoded; charset=UTF-8",
                data: {
                    cpf: cpf
